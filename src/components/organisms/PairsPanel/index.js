...
class PairsList extends React.Component {
  ...
  render() {
    ...
    return (
      <div className={cx("main-item-box", $.pairsList)}>
        <header>
          <h3>{l("title")}</h3>
          {isSearchMode ? (
            <input
          ...
          <table>
            <thead>
              <tr>
                ...
                <td className="text-right">{l("columns.price")}</td>
  ...
  _renderRow = ({ pair, current, changePercent, volume, favorite, format }) => {
    ...
    return (
      <tr key={pair} className={rowClassName} onClick={() => selectPair(pair)}>
        <td
        ...
        <td>{pair}</td>
        <td className="text-right">
          {isUndef(current) ? "-" : numeral(current).format(format)}
        </td>
        {!isVolumeMode && (
          <td className="text-right">
            {isUndef(changePercent) ? "-" : changePercent.toFixed(2) + "%"}
          </td>
        )}
        {isVolumeMode && (
          <td className="text-right">
