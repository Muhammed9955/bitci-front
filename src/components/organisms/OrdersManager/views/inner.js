...
class Inner extends React.Component {
  ...
  render() {
    const innerClassName = cs('row justify-content-between pt-2 pb-4 pl-5 pr-5 mr-0', $.inner)
  ...
  _renderBuySide = () => {
    ...
    return (
      <div className={sideClassName}>
        <OrderForm type={type} size={size} limit={limit} stop={stop} onSubmit={this._placeBuyOrder}
                   onChange={this._setBuyField} green submitTxt={`BUY ${currency}`}/>
      </div>
    )
  }
  ...
  _renderSellSide = () => {
    ...
    return (
      <div className={sideClassName}>
        <OrderForm type={type} size={size} limit={limit} stop={stop} onSubmit={this._placeSellOrder}
                   onChange={this._setSellField} red submitTxt={`SELL ${currency}`}/>
      </div>
    )
  }
  ...
  _set = (form, field, eventOrValue) => {
    let value = null
    if(typeof eventOrValue.target !== 'undefined') value = eventOrValue.target.value
