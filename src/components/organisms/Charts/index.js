...
class Charts extends React.Component {
  ...
  render() {
    ...
    const chartsClassName = cs(
      "main-item-box",
      fullScreenMode ? $.fsCharts : $.nfsCharts
    );
          ...
          <ul className="box-list-items">
            {openedChart === CHARTS.CANDLE && (
              <li>
                <Toggle
                  ...
                  label={l("ordersLines")}
                  active={ordersLines}
                  onChange={this._toggleOrdersLines}
                />
              </li>
            )}
            {openedChart === CHARTS.CANDLE && (
              <li>
                <IndicatorsMenu
                  indicator={indicator}
                  onChange={(i) => this._setIndicator(i)}
                />
              </li>
            )}
            <li
              className="dropdown default-dropdown-item"
              style={paddingTop1PxStyle}
            >
              <a
                tabIndex="0"
                role="button"
                onClick={() => this._openChart(CHARTS.CANDLE)}
              >
            ...
            >
              <a
                tabIndex="0"
                role="button"
                onClick={() => this._openChart(CHARTS.DEPTH)}
              >
                <span>{l("depth")}</span>
              </a>
            </li>
            ...
            <li>
              <a
                ...
                tabIndex="0"
                onClick={this._toggleFullScreenMode}
              />
            </li>
          </ul>
        </header>
  ...
  _renderChart() {
    const { openedChart, chartWrapperHeight, indicator, ordersLines } =
      this.state;

    if (openedChart === CHARTS.DEPTH)
      return <DepthChart height={chartWrapperHeight} />;
    if (openedChart === CHARTS.CANDLE)
      return (
        <CandleStickChart
          height={chartWrapperHeight}
          indicator={indicator}
          showOrders={ordersLines}
        />
      );
  }
  ...
  _setIndicator = (indicator) => this.setState({ indicator });
}
