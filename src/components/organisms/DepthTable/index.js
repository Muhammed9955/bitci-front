  ...
  render() {
    ...
    return (
      <div className={$.depthTable} ref={(el) => this._container = el}>
        {cutList.map((line) => (
          <div key={`${line.price}${line.size}`} className={lineClass} onClick={() => onLineClick(line)}>
            <div className={$.column}>{format(line.price)}</div>
            <div className={$.column}>{line.size}</div>
            <Desktop component='div' className={$.column}>{line.total.toFixed(5)}</Desktop>
            <div className={$.bar} style={{transform: `scaleX(${line.size / maxSize})`, transformOrigin: 'left'}}/>
          </div>
        ))}
      </div>
    )
...
export const SellDepthTable = connectData(DATA_TYPES.COLL_DEPTH_DATA_SELL, mapDataToProps)(ConnectedDepthTable)
export const BuyDepthTable = connectData(DATA_TYPES.COLL_DEPTH_DATA_BUY, mapDataToProps)(ConnectedDepthTable)